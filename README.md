...
4. 선택한 Preset이 저장되어 있고 Sharp 센서가 3초 동안 감지되면 해당
   Preset의 전체 Step을 한 번 실행합니다.
5. 실행 중 또는 완료 직후 물체가 계속 감지되면 재실행하지 않습니다.
   물체가 센서 범위에서 사라진 뒤 다시 3초 감지되면 재실행됩니다.

센서 감지 조건은 `AX_SLAVE`에서 10~30cm 범위의 샘플 3회 연속 확인으로
판정합니다. 감지 중단 또는 슬레이브 상태 회신 타임아웃 시 카운트다운은
초기화됩니다.
