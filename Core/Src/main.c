...
/* USER CODE BEGIN PV */
static AX12_AppState ax12_app;
static uint32_t ax12_last_telemetry_ms = 0U;
/* USER CODE END PV */
...
static void AX12_PublishTelemetry(const AX12_AppState *app)
{
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if ((ax12_app.ready) && ((HAL_GetTick() - ax12_last_telemetry_ms) >= 100U))
    {
      ax12_last_telemetry_ms = HAL_GetTick();
      AX12_PublishTelemetry(&ax12_app);
    }
